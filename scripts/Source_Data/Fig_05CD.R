...
NOD2_CM$p + theme_bw() + labs(y = "Residualised NOD2 expression", x = "Normalised Classical Monocyte proportion", colour = "Genotype:\n16:50652646:A:C") +
theme(axis.text.x = element_text(angle = 0, size = 15, colour = "black"),
    axis.text.y = element_text(angle = 0, size = 15, colour = "black"),
    axis.title.y = element_text(size = 21, color = "black"),
    ...
    legend.text = element_text(size = 12)) + guides(colour = guide_legend(override.aes = list(size=5))) +
    scale_colour_viridis(discrete = TRUE)
write.table(NOD2_CM$exp, file = "/home/workspace/jogrady/heQTL/results/Response_1/Source_Data/Fig_5C.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
ATG4B$p + theme_bw() + labs(y = "Residualised ATG4B expression", x = "Normalised non-classical monocyte proportion", colour = "Genotype:\n2:241678377:A:G") +
theme(axis.text.x = element_text(angle = 0, size = 15, colour = "black"),
    axis.text.y = element_text(angle = 0, size = 15, colour = "black"),
    axis.title.y = element_text(size = 21, color = "black"),
    axis.title.x = element_text(size = 21, color = "black"),
...
write.table(NOD2_CM$exp, file = "/home/workspace/jogrady/heQTL/results/Response_1/Source_Data/Fig_5C.txt", sep = "\t", quote = FALSE, row.names = FALSE)
write.table(ATG4B$exp, file = "/home/workspace/jogrady/heQTL/results/Response_1/Source_Data/Fig_5D.txt", sep = "\t", quote = FALSE, row.names = FALSE)