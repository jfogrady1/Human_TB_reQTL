...
#rule all:
 #   input:
  #      expand('{name}/{region}.{preset}.vcf.gz',name=config['Run_merged'],region=config['regions'],preset=config['GL_config']),
   #     expand('work/RNA_seq/variant_call/mergedcall/deepvariant/{sample}.{region}.g.vcf.gz',sample=cohort_samples, region = config["regions"], allow_missing=True)
...
rule deepvariant_make_examples_merged:
    input:
        ...
        bam = lambda wildcards: multiext(f'work/RNA_seq/align/mergedfinal/{config["samples_merged"][wildcards.sample]}','','.bai')
    output:
        examples = temp('work/RNA_seq/variant_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/make_examples.tfrecord-{N}-of-{sharding}.gz'),
        gvcf = temp('work/RNA_seq/variant_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/gvcf.tfrecord-{N}-of-{sharding}.gz'),
        info = temp('work/RNA_seq/variant_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/make_examples.tfrecord-{N}-of-{sharding}.gz.example_info.json')
    params:
...
rule deepvariant_call_variants_merged:
    input:
        expand('work/RNA_seq/variant_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/make_examples.tfrecord-{N}-of-{sharding}.gz',sharding=f'{config["shards"]:05}',N=[f'{i:05}' for i in range(config['shards'])],allow_missing=True)
    output:
        temp('work/RNA_seq/varaint_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/call_variants_output.tfrecord.gz')
    params:
...
rule deepvariant_postprocess_merged:
    input:
        ...
        gvcf = expand('work/RNA_seq/variant_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/gvcf.tfrecord-{N}-of-{sharding}.gz',sharding=f'{config["shards"]:05}',N=[f'{i:05}' for i in range(config['shards'])],allow_missing=True)
    output:
        vcf = multiext('work/RNA_seq/variant_call/mergedcall/deepvariant/{sample}.{region}.vcf.gz','','.tbi'),
        gvcf = multiext('work/RNA_seq/variant_call/mergedcall/deepvariant/{sample}.{region}.g.vcf.gz','','.tbi')
    params:
...
rule GLnexus_merge_merged:
    input:
        gvcfs = expand('work/RNA_seq/variant_call/mergedcall/deepvariant/{sample}.{region}.g.vcf.gz',sample=cohort_samples, region = config["regions"],allow_missing=True),
        tbi = expand('work/RNA_seq/variant_call/mergedcall/deepvariant/{sample}.{region}.g.vcf.gz.tbi',sample=cohort_samples,region = config["regions"], allow_missing=True)
    output:
        multiext('work/RNA_seq/variant_call/mergedcall/{region}.{preset}.vcf.gz','','.tbi')
    params:
