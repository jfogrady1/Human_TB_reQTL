...
config.setdefault('Run_merged', 'DV_variant_calling')
#if 'binding_paths' in config:
 #   for path in config['binding_paths']:
  #      workflow.singularity_args += f' -B {path}'

cohort_samples = config['samples_merged'] if 'glob' not in config['samples_merged'] else glob_wildcards(config["bam_path_merged"] + config["samples_merged"]["glob"]).sample
#rule all:
 #   input:
  #      expand('{name}/{region}.{preset}.vcf.gz',name=config['Run_merged'],region=config['regions'],preset=config['GL_config']),
   #     expand('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/deepvariant/{sample}.{region}.g.vcf.gz',sample=cohort_samples, region = config["regions"], allow_missing=True)
...
rule deepvariant_make_examples_merged:
    input:
        reference = multiext(config['reference'],'','.fai'),
        bam = lambda wildcards: multiext(f'/home/workspace/jogrady/heQTL/work/RNA_seq/align/mergedfinal/{config["samples_merged"][wildcards.sample]}','','.bai')
    output:
        examples = temp('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/make_examples.tfrecord-{N}-of-{sharding}.gz'),
        gvcf = temp('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/gvcf.tfrecord-{N}-of-{sharding}.gz'),
        info = temp('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/make_examples.tfrecord-{N}-of-{sharding}.gz.example_info.json')
    params:
...
rule deepvariant_call_variants_merged:
    input:
        expand('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/make_examples.tfrecord-{N}-of-{sharding}.gz',sharding=f'{config["shards"]:05}',N=[f'{i:05}' for i in range(config['shards'])],allow_missing=True)
    output:
        temp('/home/workspace/jogrady/heQTL/work/RNA_seq/varaint_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/call_variants_output.tfrecord.gz')
    params:
...
rule deepvariant_postprocess_merged:
    input:
        reference = multiext(config['reference'],'','.fai'),
        variants = rules.deepvariant_call_variants_merged.output[0],
        gvcf = expand('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/deepvariant/intermediate_results_{sample}_{region}/gvcf.tfrecord-{N}-of-{sharding}.gz',sharding=f'{config["shards"]:05}',N=[f'{i:05}' for i in range(config['shards'])],allow_missing=True)
    output:
        vcf = multiext('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/deepvariant/{sample}.{region}.vcf.gz','','.tbi'),
        gvcf = multiext('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/deepvariant/{sample}.{region}.g.vcf.gz','','.tbi')
    params:
...
rule GLnexus_merge_merged:
    input:
        gvcfs = expand('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/deepvariant/{sample}.{region}.g.vcf.gz',sample=cohort_samples, region = config["regions"],allow_missing=True),
        tbi = expand('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/deepvariant/{sample}.{region}.g.vcf.gz.tbi',sample=cohort_samples,region = config["regions"], allow_missing=True)
    output:
        multiext('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/mergedcall/{region}.{preset}.vcf.gz','','.tbi')
    params:
