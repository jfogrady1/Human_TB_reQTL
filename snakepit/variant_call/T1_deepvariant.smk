...
config.setdefault('Run_name_1', 'DV_variant_calling')
...
#rule all:
 #   input:
  #      expand('{name}/{region}.{preset}.vcf.gz',name=config['Run_name_1'],region=config['regions'],preset=config['GL_config'])

cohort_samples = config['samples_1'] if 'glob' not in config['samples_1'] else glob_wildcards(config["bam_path_1"] + config["samples_1"]["glob"]).sample
...
rule deepvariant_postprocess_T1:
    input:
        reference = multiext(config['reference'],'','.fai'),
