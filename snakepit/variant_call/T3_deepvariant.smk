...
config.setdefault('Run_name_3', 'DV_variant_calling')
...
#rule all:
 #   input:
  #      expand('{name}/{region}.{preset}.vcf.gz',name=config['Run_name_3'],region=config['regions'],preset=config['GL_config'])

cohort_samples = config['samples_3'] if 'glob' not in config['samples_3'] else glob_wildcards(config["bam_path_3"] + config["samples_3"]["glob"]).sample
...
rule deepvariant_make_examples_T3:
    input:
        reference = multiext(config['reference'],'','.fai'),
        bam = lambda wildcards: multiext(f'/home/workspace/jogrady/heQTL/work/RNA_seq/align/T3/{config["samples_3"][wildcards.sample]}','','.bai')
    output:
        examples = temp('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/T3/deepvariant/intermediate_results_{sample}_{region}/make_examples.tfrecord-{N}-of-{sharding}.gz'),
        gvcf = temp('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/T3/deepvariant/intermediate_results_{sample}_{region}/gvcf.tfrecord-{N}-of-{sharding}.gz'),
        info = temp('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/T3/deepvariant/intermediate_results_{sample}_{region}/make_examples.tfrecord-{N}-of-{sharding}.gz.example_info.json')
    params:
...
rule deepvariant_call_variants_T3:
    input:
        expand('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/T3/deepvariant/intermediate_results_{sample}_{region}/make_examples.tfrecord-{N}-of-{sharding}.gz',sharding=f'{config["shards"]:05}',N=[f'{i:05}' for i in range(config['shards'])],allow_missing=True)
    output:
        temp('/home/workspace/jogrady/heQTL/work/RNA_seq/varaint_call/T3/deepvariant/intermediate_results_{sample}_{region}/call_variants_output.tfrecord.gz')
    params:
...
rule deepvariant_postprocess_T3:
    input:
        reference = multiext(config['reference'],'','.fai'),
        variants = rules.deepvariant_call_variants_T3.output[0],
        gvcf = expand('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/T3/deepvariant/intermediate_results_{sample}_{region}/gvcf.tfrecord-{N}-of-{sharding}.gz',sharding=f'{config["shards"]:05}',N=[f'{i:05}' for i in range(config['shards'])],allow_missing=True)
    output:
        vcf = multiext('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/T3/deepvariant/{sample}.{region}.vcf.gz','','.tbi'),
        gvcf = multiext('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/T3/deepvariant/{sample}.{region}.g.vcf.gz','','.tbi')
    params:
...
rule GLnexus_merge_T3:
    input:
        gvcfs = expand('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/T3/deepvariant/{sample}.{region}.g.vcf.gz',sample=cohort_samples,region = config["regions"],allow_missing=True),
        tbi = expand('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/T3/deepvariant/{sample}.{region}.g.vcf.gz.tbi',sample=cohort_samples,region = config["regions"],allow_missing=True)
    output:
        multiext('/home/workspace/jogrady/heQTL/work/RNA_seq/variant_call/T3/{region}.{preset}.vcf.gz','','.tbi')
    params:
